	...
	{#if mounted}
		...
		<div class="relative inline-block" in:fly={{ y: 30, delay: 600, duration: 600, easing: bounceOut }}>
			<Button
				...
				class="relative transform rounded-full bg-gradient-to-r from-blue-500 to-purple-600 px-4 py-2 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:scale-105 hover:from-blue-600 hover:to-purple-700 hover:shadow-xl sm:px-6 sm:py-3 sm:text-lg {isLoading ? 'animate-pulse' : ''}"
				disabled={isLoading}
			>
				<div class="flex items-center gap-1 sm:gap-2">
					<Shuffle class="h-4 w-4 sm:h-5 sm:w-5 {isLoading ? 'animate-spin' : ''}" />
					<span class="hidden sm:inline">{isLoading ? 'Discovering...' : buttonText}</span>
					<span class="sm:hidden">{isLoading ? 'Finding...' : 'Discover'}</span>
